...
public class FortanixDsmTest {
  ...
  private static final Bytes EXPECTED_KEY = Bytes.fromHexString("5ec00b6842e0021be1dbacb18b8e5c834cb41fbbf8f7857ee43f1c1ccbe63c4f");
  ...
  void fetchExistingSecretValueFromFortanixDsm() {
    final FortanixDSM fortanixDsm =
        FortanixDSM.createWithApiKeyCredential(SERVER, API_KEY, false, false);
  ...
  void fetchNonExistingSecretReturnsEmpty() {
    final FortanixDSM fortanixDsm =
        FortanixDSM.createWithApiKeyCredential(SERVER, API_KEY, false, false);
    final Optional<Bytes> secretKey = fortanixDsm.fetchSecret(KEY_ID.substring(0, KEY_ID.length()-1));
  ...
  void connectingWithInvalidApiKeyIdThrowsException() {    
    assertThatExceptionOfType(RuntimeException.class)
        .isThrownBy(() -> FortanixDSM.createWithApiKeyCredential(SERVER, API_KEY.substring(0, API_KEY.length()-1), false, false))
  ...
  void secretsCanBeMappedUsingCustomMappingFunction() {
    ...
    Collection<SimpleEntry<String, Bytes>> entries = fortanixDsm.mapSecret(KEY_ID, SimpleEntry::new);
